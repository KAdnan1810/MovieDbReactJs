...
function Card() {
  ...
  const selectPageHandler = (selectedPage) => {
    if (selectedPage >= 1 && selectedPage <= upComing.length / 10 && selectedPage !== page) {
  ...
  return (
    <>
      <div className=' grid grid-cols-4 m-2 p-2 gap-9'>
          { upComing.slice(page * 10 - 10, page * 10).map((movie,data) => 
      ...
      {upComing.length > 0 && <div className="pagination">
      <span onClick={() => selectPageHandler(page - 1)} className={page > 1 ? "" : "pagination__disable"}>◀</span>
  
      {[...Array(upComing.length / 10)].map((_, i) => {
        return <span key={i} className={page === i + 1 ? "pagination__selected" : ""} onClick={() => selectPageHandler(i + 1)}>{i + 1}</span>
      })}
  
      <span onClick={() => selectPageHandler(page + 1)} className={page < upComing.length / 10 ? "" : "pagination__disable"}>▶</span>
      </div>}
    </>
  )
}
